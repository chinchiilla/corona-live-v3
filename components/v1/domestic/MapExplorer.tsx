...
const Label = ({ pos, name, stats, cityId }) => {
    ...
    return (
        <SLabel color={deltaColor as ThemeType} onClick={onClick} style={{ left, top, opacity: cases == 0 ? 0.4 : 1 }}>
    ...
    useEffect(() => {
        ...
        svg.select(".regions")
            ...
            .join(
                ...
                (update) =>
                    update.attr("stroke", currentTheme["mapBorder"]).attr("fill", (d) => {
                        const opacity = 0.1 + Math.max(stats[statType][d.properties.code][0] / maxCases - 0.1, 0);
                        return `rgb(86, 115, 235,${opacity})`;
                    }),
            ...
            <Wrapper fadeInUp className='container'>
                {features &&
                    features.map((f, i) => {
                        ...
                        if (!stats[statType][code][0]) return <></>;
                        return (
                            <Label
                                key={code}
                                pos={pos}
